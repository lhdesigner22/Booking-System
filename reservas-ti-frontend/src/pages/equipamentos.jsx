      ...
      <div className="page-layout">
        ...
        <main className="main-content">
          ...
          >
            ...
            <motion.button
              onClick={() => setFiltroDisp(f => f === 'disponivel' ? 'todos' : 'disponivel')}
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.96 }}
              title={filtroDisp === 'disponivel' ? 'Mostrar indisponíveis' : 'Ocultar indisponíveis'}
              style={{
                display: 'flex', alignItems: 'center', gap: 7,
                padding: '6px 13px', borderRadius: 99, fontSize: 12, fontWeight: 600,
                cursor: 'pointer', border: '1.5px solid',
                transition: 'all 0.18s',
                background: filtroDisp === 'disponivel'
                  ? 'rgba(248,113,113,0.12)' : 'transparent',
                borderColor: filtroDisp === 'disponivel'
                  ? 'rgba(248,113,113,0.4)' : 'var(--border)',
                color: filtroDisp === 'disponivel'
                  ? '#F87171' : 'var(--text-muted)',
              }}
            >
              {filtroDisp === 'disponivel' ? (
                <>
